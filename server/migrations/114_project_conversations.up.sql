...
CREATE INDEX idx_chat_session_project
    ON chat_session(workspace_id, creator_id, project_id, updated_at DESC)
    WHERE project_id IS NOT NULL;

ALTER TABLE agent_task_queue
    ADD COLUMN project_id UUID REFERENCES project(id) ON DELETE SET NULL;

CREATE INDEX idx_agent_task_queue_project
    ON agent_task_queue(project_id)
    WHERE project_id IS NOT NULL;

-- Chat tasks can now carry their own project snapshot. Issue-bound tasks keep
-- deriving project from issue.project_id, while chat tasks use atq.project_id.
           ...
           OR OLD.project_id IS DISTINCT FROM NEW.project_id THEN
            IF OLD.runtime_id IS NOT NULL THEN
                ...
                SELECT DISTINCT
                    ...
                    OLD.runtime_id,
                    OLD.agent_id,
                    COALESCE(OLD.project_id, i_old.project_id),
                    tu.provider,
                    tu.model
                  FROM task_usage tu
            ...
            IF NEW.runtime_id IS NOT NULL THEN
                ...
                SELECT DISTINCT
                    ...
                    NEW.runtime_id,
                    NEW.agent_id,
                    COALESCE(NEW.project_id, i_new.project_id),
                    tu.provider,
                    tu.model
                  FROM task_usage tu
    ...
    ELSIF TG_OP = 'DELETE' THEN
        IF OLD.runtime_id IS NOT NULL THEN
            ...
            SELECT DISTINCT
                ...
                OLD.runtime_id,
                OLD.agent_id,
                COALESCE(OLD.project_id, i.project_id),
                tu.provider,
                tu.model
              FROM task_usage tu
              JOIN agent a ON a.id = OLD.agent_id
...
BEFORE UPDATE OF runtime_id, issue_id, project_id OR DELETE ON agent_task_queue
...
BEGIN
    ...
    SELECT
        ...
        atq.runtime_id,
        atq.agent_id,
        COALESCE(atq.project_id, i.project_id),
        OLD.provider,
        OLD.model
      FROM agent_task_queue atq
...
BEGIN
    ...
    dirty_from_updates AS (
        SELECT DISTINCT
            ...
            COALESCE(atq.project_id, i.project_id) AS project_id,
    ...
    recomputed AS (
        SELECT
         ...
         WHERE (COALESCE(atq.project_id, i.project_id) IS NOT DISTINCT FROM dk.project_id)
