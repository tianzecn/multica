...
func readOpenclawStdout(r io.Reader, idleGrace time.Duration) (buf []byte, cutShort bool, err error) {
	...
	for {
		...
		case <-ticker.C:
			// Keep the idle decision and buffer snapshot under the same lock. If
			// the reader appends bytes between those operations, an old lastByte
			// paired with a new, now-parseable buffer can falsely report a clean
			// stream as cut short just before EOF.
			mu.Lock()
			if atEOF {
				mu.Unlock()
				continue // let the <-finished branch report the final state
			}
			if len(acc) == 0 || time.Since(lastByte) < idleGrace {
				mu.Unlock()
				continue
			}
