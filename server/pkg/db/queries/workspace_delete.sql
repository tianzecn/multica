...
-- name: DeleteWorkspaceChannelData :exec
-- Channel collaboration spans its own graph plus chat, task, issue, agent and
-- project references. Delete the channel-owned side first, in dependency
-- order, so the later workspace-root steps never need FK cascades to finish
-- local channel cleanup. Each CTE references the previous DELETE to make the
-- ordering explicit inside this single statement.
WITH
ws_channels AS MATERIALIZED (
    SELECT id FROM channel WHERE channel.workspace_id = sqlc.arg('workspace_id')
),
ws_plans AS MATERIALIZED (
    SELECT id FROM channel_dispatch_plan
    WHERE channel_id IN (SELECT id FROM ws_channels)
),
deleted_feedback AS (
    DELETE FROM channel_dispatch_feedback
    WHERE plan_id IN (SELECT id FROM ws_plans)
    RETURNING id
),
deleted_runs AS (
    DELETE FROM channel_agent_run
    WHERE channel_id IN (SELECT id FROM ws_channels)
      AND (SELECT count(*) FROM deleted_feedback) >= 0
    RETURNING id
),
deleted_steps AS (
    DELETE FROM channel_dispatch_step
    WHERE channel_id IN (SELECT id FROM ws_channels)
      AND (SELECT count(*) FROM deleted_runs) >= 0
    RETURNING id
),
deleted_plans AS (
    DELETE FROM channel_dispatch_plan
    WHERE channel_id IN (SELECT id FROM ws_channels)
      AND (SELECT count(*) FROM deleted_steps) >= 0
    RETURNING id
),
deleted_threads AS (
    DELETE FROM channel_agent_thread
    WHERE channel_id IN (SELECT id FROM ws_channels)
      AND (SELECT count(*) FROM deleted_plans) >= 0
    RETURNING id
),
deleted_read_states AS (
    DELETE FROM channel_read_state
    WHERE channel_id IN (SELECT id FROM ws_channels)
      AND (SELECT count(*) FROM deleted_threads) >= 0
    RETURNING channel_id
),
deleted_issue_links AS (
    DELETE FROM issue_channel
    WHERE channel_id IN (SELECT id FROM ws_channels)
      AND (SELECT count(*) FROM deleted_read_states) >= 0
    RETURNING channel_id
),
deleted_approvals AS (
    DELETE FROM approval_request
    WHERE (approval_request.workspace_id = sqlc.arg('workspace_id') OR channel_id IN (SELECT id FROM ws_channels))
      AND (SELECT count(*) FROM deleted_issue_links) >= 0
    RETURNING id
),
deleted_messages AS (
    DELETE FROM channel_message
    WHERE channel_id IN (SELECT id FROM ws_channels)
      AND (SELECT count(*) FROM deleted_approvals) >= 0
    RETURNING id
),
deleted_members AS (
...
deleted_channels AS (
    DELETE FROM channel
    WHERE channel.workspace_id = sqlc.arg('workspace_id')
      AND (SELECT count(*) FROM deleted_sessions) >= 0
    RETURNING id
)
DELETE FROM channel_group
WHERE channel_group.workspace_id = sqlc.arg('workspace_id')
  AND (SELECT count(*) FROM deleted_channels) >= 0;

-- name: DeleteWorkspaceProjectLinks :exec
-- These project-owned rows must disappear before the later runtime, project
-- and pull-request root deletes. project_pull_request has no workspace_id, so
-- resolve it through the workspace's project set.
WITH
ws_projects AS MATERIALIZED (
    SELECT id FROM project WHERE project.workspace_id = sqlc.arg('workspace_id')
),
deleted_device_bindings AS (
    DELETE FROM project_device_binding
    WHERE project_device_binding.workspace_id = sqlc.arg('workspace_id')
       OR project_id IN (SELECT id FROM ws_projects)
    RETURNING id
),
deleted_workspace_configs AS (
    DELETE FROM project_workspace_config
    WHERE project_workspace_config.workspace_id = sqlc.arg('workspace_id')
       OR project_id IN (SELECT id FROM ws_projects)
    RETURNING project_id
)
DELETE FROM project_pull_request
WHERE project_id IN (SELECT id FROM ws_projects)
  AND (SELECT count(*) FROM deleted_device_bindings) >= 0
  AND (SELECT count(*) FROM deleted_workspace_configs) >= 0;
