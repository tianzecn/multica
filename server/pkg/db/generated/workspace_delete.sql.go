...
const deleteWorkspaceChannelData = `-- name: DeleteWorkspaceChannelData :exec
WITH
ws_channels AS MATERIALIZED (
    SELECT id FROM channel WHERE channel.workspace_id = $1
),
...
deleted_approvals AS (
    ...
    WHERE (approval_request.workspace_id = $1 OR channel_id IN (SELECT id FROM ws_channels))
...
WHERE channel_group.workspace_id = $1
...
func (q *Queries) DeleteWorkspaceChannelData(ctx context.Context, workspaceID pgtype.UUID) error {
	_, err := q.db.Exec(ctx, deleteWorkspaceChannelData, workspaceID)
	return err
}
...
const deleteWorkspaceProjectLinks = `-- name: DeleteWorkspaceProjectLinks :exec
WITH
ws_projects AS MATERIALIZED (
    SELECT id FROM project WHERE project.workspace_id = $1
),
deleted_device_bindings AS (
    DELETE FROM project_device_binding
    WHERE project_device_binding.workspace_id = $1
       OR project_id IN (SELECT id FROM ws_projects)
    RETURNING id
),
deleted_workspace_configs AS (
    DELETE FROM project_workspace_config
    WHERE project_workspace_config.workspace_id = $1
       OR project_id IN (SELECT id FROM ws_projects)
...
func (q *Queries) DeleteWorkspaceProjectLinks(ctx context.Context, workspaceID pgtype.UUID) error {
	_, err := q.db.Exec(ctx, deleteWorkspaceProjectLinks, workspaceID)
	return err
}
