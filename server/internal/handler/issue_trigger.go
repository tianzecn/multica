...
func (h *Handler) dispatchIssueRun(ctx context.Context, issue db.Issue, trigger service.IssueRunTrigger, actorType, actorID, handoffNote string, taskOpts service.EnqueueIssueTaskOptions) {
	...
	case "agent":
		...
		_, _ = h.TaskService.EnqueueTaskForIssueWithHandoffAndOptions(ctx, issue, handoffNote, memberActorUserID(actorType, actorID), taskOpts)
	case "squad":
		h.enqueueSquadLeaderTask(ctx, issue, pgtype.UUID{}, actorType, actorID, handoffNote, taskOpts)
	}
}
