...
func (h *Handler) CreateComment(w http.ResponseWriter, r *http.Request) {
	...
	resp.TriggerOutcomes = h.triggerTasksForComment(r.Context(), issue, comment, parentComment, authorType, authorID, originatorUserID, delegationAuthority, suppressAgentIDs, service.EnqueueIssueTaskOptions{ProjectContinueOnDirty: req.ProjectContinueOnDirty})
...
func (h *Handler) triggerTasksForComment(ctx context.Context, issue db.Issue, comment db.Comment, parentComment *db.Comment, actorType, actorID, originatorUserID, delegationAuthorityUserID string, suppressAgentIDs []pgtype.UUID, opts ...service.EnqueueIssueTaskOptions) []CommentTriggerOutcome {
	...
	taskOpts := service.EnqueueIssueTaskOptions{}
	if len(opts) > 0 {
		taskOpts = opts[0]
	}
	enqueued := h.enqueueCommentAgentTriggers(ctx, issue, comment.ID, triggers, taskOpts)
...
func (h *Handler) enqueueCommentAgentTriggers(ctx context.Context, issue db.Issue, triggerCommentID pgtype.UUID, triggers []commentAgentTrigger, opts ...service.EnqueueIssueTaskOptions) map[string]commentEnqueueResult {
	taskOpts := service.EnqueueIssueTaskOptions{}
	if len(opts) > 0 {
		taskOpts = opts[0]
	}
	...
	for _, trigger := range triggers {
		status, reason := h.resolveCommentTriggerEnqueue(ctx, issue, trigger, triggerCommentID, getEscalationDelay, taskOpts)
...
func (h *Handler) resolveCommentTriggerEnqueue(ctx context.Context, issue db.Issue, trigger commentAgentTrigger, triggerCommentID pgtype.UUID, getEscalationDelay func() time.Duration, opts ...service.EnqueueIssueTaskOptions) (DispatchStatus, DispatchReasonCode) {
	taskOpts := service.EnqueueIssueTaskOptions{}
	if len(opts) > 0 {
		taskOpts = opts[0]
	}
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		if err := h.enqueueSingleCommentTrigger(ctx, issue, triggerCommentID, trigger, getEscalationDelay, taskOpts); err != nil {
...
func (h *Handler) propagateUncoveredCommentObligation(ctx context.Context, issue db.Issue, trigger commentAgentTrigger, commentID pgtype.UUID, headSha pgtype.Text) bool {
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		err = h.enqueueSingleCommentTrigger(ctx, issue, commentID, trigger, noEscalation, service.EnqueueIssueTaskOptions{})
		if err == nil {
			return true
		}
		if !errors.Is(err, service.ErrDuplicatePendingTask) {
...
func (h *Handler) enqueueSingleCommentTrigger(ctx context.Context, issue db.Issue, triggerCommentID pgtype.UUID, trigger commentAgentTrigger, getEscalationDelay func() time.Duration, opts ...service.EnqueueIssueTaskOptions) error {
	taskOpts := service.EnqueueIssueTaskOptions{}
	if len(opts) > 0 {
		taskOpts = opts[0]
	}
	...
	case commentTriggerSourceIssueAssignee:
		if trigger.Squad != nil {
			if _, err := h.TaskService.EnqueueTaskForSquadLeaderWithOptions(ctx, issue, trigger.Agent.ID, trigger.Squad.ID, triggerCommentID, taskOpts); err != nil {
		...
		if _, err := h.TaskService.EnqueueTaskForIssueWithOptions(ctx, issue, taskOpts, triggerCommentID); err != nil {
	...
	case commentTriggerSourceMentionAgent:
		if _, err := h.TaskService.EnqueueTaskForMentionWithOptions(ctx, issue, trigger.Agent.ID, triggerCommentID, taskOpts); err != nil {
	...
	case commentTriggerSourceThreadParent, commentTriggerSourceConversation:
		...
		if trigger.Source == commentTriggerSourceConversation && trigger.Squad != nil {
			task, err = h.TaskService.EnqueueTaskForSquadLeaderWithOptions(ctx, issue, trigger.Agent.ID, trigger.Squad.ID, triggerCommentID, taskOpts)
		} else {
			task, err = h.TaskService.EnqueueTaskForThreadParentWithOptions(ctx, issue, trigger.Agent.ID, triggerCommentID, taskOpts)
		}
		if err != nil {
...
func (h *Handler) UpdateComment(w http.ResponseWriter, r *http.Request) {
	...
	retriggerEditedComment := func() []CommentTriggerOutcome {
		...
		return h.triggerTasksForComment(r.Context(), issue, comment, parentComment, actorType, actorID, h.invokeOriginatorFromRequest(r, actorType, actorID), delegationAuthority, suppressAgentIDs, service.EnqueueIssueTaskOptions{})
	}
...
func (h *Handler) retriggerCancelledTaskSurvivors(ctx context.Context, issue db.Issue, cancelled []db.AgentTaskQueue, excludedCommentID pgtype.UUID) {
	...
	for i := range comments {
		...
		if len(scoped) > 0 {
			h.enqueueCommentAgentTriggers(ctx, issue, comment.ID, scoped, service.EnqueueIssueTaskOptions{})
		}
	}
}
