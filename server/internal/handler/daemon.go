		...
		if issue, err := h.Queries.GetIssue(r.Context(), task.IssueID); err == nil {
			...
			if issue.ProjectID.Valid {
				...
				if rows := h.listProjectResourcesForProject(r.Context(), issue.ProjectID); len(rows) > 0 {
					resp.ProjectResources, projectRepos = projectResourcesForClaim(rows)
				}
			}
		...
		if cs, err := h.Queries.GetChatSession(r.Context(), task.ChatSessionID); err == nil {
			...
			if cs.ProjectID.Valid {
				if project, err := h.Queries.GetProjectInWorkspace(r.Context(), db.GetProjectInWorkspaceParams{
					...
					applyProjectWorkspaceConfigForClaim(&resp, h.projectWorkspaceConfigForClaim(r.Context(), project))
...
func (h *Handler) CompleteTask(w http.ResponseWriter, r *http.Request) {
	...
	task, err := h.TaskService.CompleteTask(r.Context(), parseUUID(taskID), result, req.SessionID, storedWorkDir, req.SessionRolloutMissing, req.RetiredSessionID)
	if err != nil {
...
func (h *Handler) FailTask(w http.ResponseWriter, r *http.Request) {
	...
	storedWorkDir := taskStoredWorkDir(originalTask, req.WorkDir)
	task, err := h.TaskService.FailTask(r.Context(), parseUUID(taskID), req.Error, req.SessionID, storedWorkDir, req.FailureReason, req.SessionRolloutMissing, req.RetiredSessionID)
	if err != nil {
