...
func buildSearchQuery(phrase string, terms []string, queryNum int, hasNum bool, includeClosed bool) (string, []any) {
	return buildSearchQueryWithProject(phrase, terms, queryNum, hasNum, includeClosed, pgtype.UUID{})
}

// buildSearchQueryWithProject builds a dynamic SQL query for issue search.
...
func buildSearchQueryWithProject(phrase string, terms []string, queryNum int, hasNum bool, includeClosed bool, projectID pgtype.UUID) (string, []any) {
	...
	if projectID.Valid {
		whereClause += fmt.Sprintf(" AND i.project_id = %s", nextArg(projectID))
	}
...
func (h *Handler) SearchIssues(w http.ResponseWriter, r *http.Request) {
	...
	if rawProjectID := strings.TrimSpace(r.URL.Query().Get("project_id")); rawProjectID != "" {
		var projectOK bool
		projectID, projectOK = parseUUIDOrBadRequest(w, rawProjectID, "project_id")
		if !projectOK {
			return
		}
	...
	sqlQuery, args := buildSearchQueryWithProject(q, terms, queryNum, hasNum, includeClosed, projectID)
...
func (h *Handler) QuickCreateIssue(w http.ResponseWriter, r *http.Request) {
	...
	task, err := h.TaskService.EnqueueQuickCreateTaskWithProjectConsent(r.Context(), wsUUID, requesterUUID, agentUUID, squadUUID, prompt, priority, dueDate, projectUUID, parentIssueUUID, attachmentIDs, req.ProjectContinueOnDirty)
	if err != nil {
...
type CreateIssueRequest struct {
	...
	// ProjectContinueOnDirty is explicit user consent to snapshot a dirty
	// project workspace before the assigned run starts.
	ProjectContinueOnDirty bool `json:"project_continue_on_dirty,omitempty"`
}
...
func (h *Handler) CreateIssue(w http.ResponseWriter, r *http.Request) {
	...
	res, err := h.IssueService.Create(r.Context(), service.IssueCreateParams{
		...
		TaskOptions:      service.EnqueueIssueTaskOptions{ProjectContinueOnDirty: req.ProjectContinueOnDirty},
...
type UpdateIssueRequest struct {
	...
	// ProjectContinueOnDirty is explicit user consent to snapshot a dirty
	// project workspace before a run triggered by this update starts.
	ProjectContinueOnDirty bool `json:"project_continue_on_dirty,omitempty"`
}
...
func (h *Handler) UpdateIssue(w http.ResponseWriter, r *http.Request) {
	...
	if trigger, ok := h.IssueService.WillEnqueueRun(r.Context(),
		...
		h.dispatchIssueRun(r.Context(), issue, trigger, actorType, actorID, req.HandoffNote, service.EnqueueIssueTaskOptions{ProjectContinueOnDirty: req.ProjectContinueOnDirty})
	}
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	for _, issueID := range req.IssueIDs {
		...
		if trigger, ok := h.IssueService.WillEnqueueRun(r.Context(),
			...
			h.dispatchIssueRun(r.Context(), issue, trigger, actorType, actorID, req.Updates.HandoffNote, service.EnqueueIssueTaskOptions{ProjectContinueOnDirty: req.Updates.ProjectContinueOnDirty})
		}
