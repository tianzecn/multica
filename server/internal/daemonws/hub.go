...
// RequestProjectWorkspace sends one project-workspace request to a single
// connected daemon runtime and waits for the response carrying the same
// request ID. Mutating operations are deliberately never broadcast.
func (h *Hub) RequestProjectWorkspace(ctx context.Context, runtimeID string, req protocol.DaemonProjectWorkspaceRequestPayload) (*protocol.DaemonProjectWorkspaceResponsePayload, error) {
	...
	response := make(chan protocol.DaemonProjectWorkspaceResponsePayload, 1)
	h.relayMu.Lock()
	if h.relayPending == nil {
		h.relayPending = make(map[string]chan protocol.DaemonProjectWorkspaceResponsePayload)
	}
	h.relayPending[req.RequestID] = response
	h.relayMu.Unlock()
	defer func() {
		h.relayMu.Lock()
...
func (h *Hub) sendProjectWorkspaceFrame(runtimeID string, frame []byte) error {
	var selected *client
	h.mu.RLock()
	for candidate := range h.byRuntime[runtimeID] {
		selected = candidate
		break
	}
	h.mu.RUnlock()
	...
	if selected.trySend(frame) {
		return nil
	}

	h.unregister(selected)
...
func (h *Hub) completeProjectWorkspaceResponse(resp protocol.DaemonProjectWorkspaceResponsePayload) {
	...
	h.relayMu.Lock()
	response := h.relayPending[resp.RequestID]
	if response != nil {
...
func (c *client) readPump() {
	...
	// Project workspace responses can contain bounded file contents and diffs;
	// this limit also comfortably contains the smaller generic RPC frames.
