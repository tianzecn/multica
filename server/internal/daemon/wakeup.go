...
func (d *Daemon) runTaskWakeupConnection(ctx context.Context, runtimeIDs []string, taskWakeups chan<- taskWakeup, runtimeSetCh <-chan struct{}) (time.Duration, error) {
	...
	sendClosed := false
	sendFrame := func(frame []byte) (*wsOutbound, error) {
		sendMu.Lock()
		defer sendMu.Unlock()
		if sendClosed {
	...
	wsRPCGeneration := d.wsRPC.attach(sendFrame)
	...
	go func() {
		errCh <- d.readTaskWakeupMessagesForConnection(ctx, conn, taskWakeups, wsRPCGeneration, sendFrame)
	}()
...
func (d *Daemon) readTaskWakeupMessages(conn *websocket.Conn, taskWakeups chan<- taskWakeup) error {
	return d.readTaskWakeupMessagesForConnection(context.Background(), conn, taskWakeups, d.wsRPC.currentGeneration(), nil)
}

func (d *Daemon) readTaskWakeupMessagesForConnection(
	ctx context.Context,
	conn *websocket.Conn,
	...
	for {
		...
		case protocol.EventDaemonProjectWorkspaceRequest:
			...
			if sendFrame == nil {
				d.logger.Debug("project workspace relay unavailable: websocket writer is not attached")
				continue
			}
			go d.handleWSProjectWorkspaceRequest(ctx, payload, sendFrame)
		}
	}
}
