...
// gateResumeToProjectWorkdir applies the same resume transparency rule to a
// daemon-bound Project workspace. Project tasks prepare a fresh metadata env
// but execute in a stable bound repository, and the server persists that cwd
// as a project:<id> marker rather than exposing the machine-local path.
func gateResumeToProjectWorkdir(task *Task, taskCtx *execenv.TaskContextForEnv, projectWorkDir string, taskLog *slog.Logger) bool {
	reused := task.PriorWorkDir == projectWorkDir ||
		task.PriorWorkDir == projectTaskWorkDirMarker(task.ProjectID)
	if !reused && task.PriorSessionID != "" {
		taskLog.Info("dropping prior session: project workdir changed, per-cwd session cannot resolve",
			"session_id", task.PriorSessionID,
			"prior_workdir", task.PriorWorkDir,
			"project_id", task.ProjectID,
		)
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	reuseTask := task
	reuseTask.PriorWorkDir = priorWorkDir
	if shouldReusePriorWorkdir(reuseTask, localAssignment, d.cfg.WorkspacesRoot) {
		var err error
	...
	reused := false
	if projectWorkDir != "" {
		reused = gateResumeToProjectWorkdir(&task, &taskCtx, projectWorkDir, taskLog)
	} else {
