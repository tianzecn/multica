...
func testDBPool(t *testing.T) *pgxpool.Pool {
	...
	// Register the pool first so its LIFO cleanup runs after every row and
	// workspace cleanup registered by the test. A per-test defer closes the
	// pool before t.Cleanup callbacks run, leaking fixtures and racing workers
...
func TestInFlightOldHeadKeepsTrailingRefresh(t *testing.T) {
	...
	m := NewManager(enabledClient(t), q, pool, func(_ context.Context, appliedPRID pgtype.UUID) {
		if appliedPRID != pr.ID {
			return
		}
		select {
		case applied <- struct{}{}:
