...
type IssueCreateOpts struct {
	...
	// TaskOptions carries explicit per-run execution consent into the task
	// created for an immediately assigned issue.
	TaskOptions EnqueueIssueTaskOptions
}
...
func (s *IssueService) Create(ctx context.Context, p IssueCreateParams, opts IssueCreateOpts) (IssueCreateResult, error) {
	...
	s.maybeEnqueueOnAssign(ctx, issue, p.CreatorType, actorID, opts.TaskOptions)
...
func (s *IssueService) maybeEnqueueOnAssign(ctx context.Context, issue db.Issue, creatorType, actorID string, taskOpts EnqueueIssueTaskOptions) {
	if !issue.AssigneeType.Valid || !issue.AssigneeID.Valid {
		return
	}
	if s.shouldEnqueueAgentTask(ctx, issue) {
		if _, err := s.TaskService.EnqueueTaskForIssueWithOptions(ctx, issue, taskOpts); err != nil {
	...
	if s.shouldEnqueueSquadLeaderOnAssign(ctx, issue) {
		s.enqueueSquadLeaderTask(ctx, issue, pgtype.UUID{}, creatorType, actorID, taskOpts)
	}
}
...
func (s *IssueService) enqueueSquadLeaderTask(ctx context.Context, issue db.Issue, triggerCommentID pgtype.UUID, authorType, authorID string, taskOpts EnqueueIssueTaskOptions) {
	squad, err := s.Queries.GetSquadInWorkspace(ctx, db.GetSquadInWorkspaceParams{
		ID:          issue.AssigneeID,
		WorkspaceID: issue.WorkspaceID,
	})
	...
	if _, err := s.TaskService.EnqueueTaskForSquadLeaderWithOptions(ctx, issue, squad.LeaderID, squad.ID, triggerCommentID, taskOpts); err != nil {
