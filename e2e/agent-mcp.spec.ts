...
import { enablePublicFeatureFlag, waitForPageText } from "./helpers";
...
test.describe("Agent MCP tab (creator-only)", () => {
  test("creator sees the MCP Apps tab and toggling a toolkit writes the allowlist", async ({
    ...
    // MCP Apps now lives in the Capabilities sub-navigation. The creator-only
    // entry is present there and opens the connection list.
