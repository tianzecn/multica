...
The [GitHub PR ↔ issue integration](/github-integration) needs the slug and webhook secret to enable Connect GitHub and accept incoming webhooks. Server-side GitHub actions such as Project repo creation, PR creation, and PR review comments also need GitHub App authentication so Multica can mint installation-scoped tokens for the selected owner.
...
| `GITHUB_APP_ID` | empty | Numeric GitHub App ID. Used with the private key to mint installation tokens for repo creation, PR creation, and PR review operations |
| `GITHUB_APP_PRIVATE_KEY` | empty | GitHub App private key PEM. Literal `\n` sequences are converted to newlines |
| `GITHUB_APP_PRIVATE_KEY_B64` | empty | Base64-encoded GitHub App private key PEM. Alternative to `GITHUB_APP_PRIVATE_KEY` |
| `GITHUB_APP_PRIVATE_KEY_PATH` | empty | Path to a GitHub App private key PEM on disk. Alternative to the env-string forms |
...
When `GITHUB_APP_ID` plus one private-key source are set, Project GitHub actions use the workspace's connected installation for the target owner. If those App auth variables are absent, self-hosted development can still fall back to `GITHUB_REPO_CREATE_TOKEN`, `GITHUB_PR_CREATE_TOKEN`, `GITHUB_PR_REVIEW_TOKEN`, or `GITHUB_TOKEN`; production deployments should prefer installation-scoped GitHub App auth.
