...
4. The daemon prepares the local working directory and invokes the corresponding AI coding tool. For a Project-bound code task, that directory is the Project's bound local folder on the target device and Multica switches it to a task branch; otherwise the daemon uses an isolated task directory. Task status becomes `running`.
