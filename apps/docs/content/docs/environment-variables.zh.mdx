...
[GitHub PR ↔ issue 集成](/github-integration) 需要 slug 和 webhook secret 来启用 Settings 里的 Connect GitHub 并接受 webhook。Project 创建 repo、创建 PR、PR review comment/resolve 这类服务端 GitHub 操作还需要 GitHub App auth，这样 Multica 才能针对用户选择的 owner mint installation-scoped token。
...
| `GITHUB_APP_ID` | 空 | 数字形式的 GitHub App ID。配合 private key 为 repo 创建、PR 创建、PR review 操作生成 installation token |
| `GITHUB_APP_PRIVATE_KEY` | 空 | GitHub App private key PEM。字面量 `\n` 会被转换成换行 |
| `GITHUB_APP_PRIVATE_KEY_B64` | 空 | Base64 编码的 GitHub App private key PEM。可替代 `GITHUB_APP_PRIVATE_KEY` |
| `GITHUB_APP_PRIVATE_KEY_PATH` | 空 | 磁盘上的 GitHub App private key PEM 路径。可替代上面两种 env 字符串形式 |
...
当 `GITHUB_APP_ID` 和任意一种 private key 来源都已配置时，Project GitHub 操作会使用该 workspace 已连接的目标 owner installation。没有这些 App auth 变量时，self-host 开发环境仍可回落到 `GITHUB_REPO_CREATE_TOKEN`、`GITHUB_PR_CREATE_TOKEN`、`GITHUB_PR_REVIEW_TOKEN` 或 `GITHUB_TOKEN`；生产部署应该优先使用 installation-scoped GitHub App auth。
