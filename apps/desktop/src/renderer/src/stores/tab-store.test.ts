...
describe("sanitizeTabPath", () => {
  ...
  it("normalizes legacy desktop aliases before they enter a tab session", () => {
    expect(sanitizeTabPath("/acme/channels")).toBe("/acme/projects");
    expect(sanitizeTabPath("/acme/runtimes")).toBe(
      "/acme/settings?tab=runtimes",
    );
    expect(sanitizeTabPath("/acme/usage")).toBe(
      "/acme/settings?tab=usage",
    );
    expect(sanitizeTabPath("/acme/projects/project-1/issues#activity")).toBe(
      "/acme/projects/project-1#activity",
    );
  });

  it("does not collapse canonical detail routes that share a legacy prefix", () => {
    expect(sanitizeTabPath("/acme/channels/channel-1")).toBe(
      "/acme/channels/channel-1",
    );
    expect(sanitizeTabPath("/acme/runtimes/runtime-1")).toBe(
