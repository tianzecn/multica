 ...
 *
 * Normalizes:
 *  1. A bare `/{slug}` (no route segment) becomes `/{slug}/issues` — the
 *     workspace's default surface.
 *  2. Legacy desktop aliases become their canonical surfaces. This keeps old
 *     persisted tabs and deep links working without an in-router `<Navigate>`
 *     (MUL-4741 invariant 1: the router never self-navigates; URLs are
 *     normalized before they become sessions).
 *
...
export function sanitizeTabPath(path: string): string | null {
  ...
  if (segments.length === 2) {
    const legacySurface = segments[1];
    if (legacySurface === "channels") {
      return `/${firstSegment}/projects${suffix}`;
    }
    if (legacySurface === "runtimes") {
      return `/${firstSegment}/settings?tab=runtimes`;
    }
    if (legacySurface === "usage") {
      return `/${firstSegment}/settings?tab=usage`;
    }
  }
  if (
    segments.length === 4 &&
    ...
    return `/${firstSegment}/projects/${segments[2]}${suffix}`;
  }
  return path;
}
