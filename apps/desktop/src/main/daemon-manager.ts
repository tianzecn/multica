...
const PROJECT_GIT_OPERATIONS = new Set([
  "fetch",
  "pull",
  "rebase",
  "commit",
...
async function fetchActiveDaemonJSON<T>(
  ...
  try {
    ...
    if (err instanceof Error) {
      throw new Error(
        err.name === "AbortError" ? "Daemon request timed out" : err.message,
      );
    }
    throw err;
  } finally {
...
export function setupDaemonManager(
  ...
  ipcMain.handle(
    "daemon:get-project-git-snapshots",
    (_event, projectId: string) =>
      fetchActiveDaemonJSON(
        projectWorkspaceLocalPath(projectId, "/git/snapshots"),
      ),
  );
  ipcMain.handle(
  ...
  ipcMain.handle(
    ...
    (_event, projectId: string, payload: unknown) =>
      fetchActiveDaemonJSON(
        projectWorkspaceLocalPath(projectId, "/files/write"),
        {
          method: "PUT",
          body: payload,
        },
