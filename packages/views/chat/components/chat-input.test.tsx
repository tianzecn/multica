  ...
  ContentEditor: forwardRef(function MockContentEditor(
    ...
    useImperativeHandle(ref, () => ({
      ...
      insertMention: (item: { id: string; label: string; type: string }) => {
        const mention = `[@${item.label}](mention://${item.type}/${item.id})`;
        valueRef.current = `${valueRef.current}${mention} `.trimStart();
      ...
      uploadFile: async (file: File) => {
        ...
        try {
          ...
          if (result) {
            ...
            valueRef.current = `${valueRef.current}![${result.filename}](${persistedURL})`.trim();
            onUpdate?.(valueRef.current);
          }
        } finally {
