...
interface ChatInputProps {
  ...
  /** Rendered just before the submit button. */
  ...
  /** Rendered inside the rounded container, above the editor. */
  topSlot?: ReactNode;
  /** Optional scoped mention list for @ completion. */
  mentionItems?: MentionItem[];
  /** Optional project scope for @ issue search inside the composer. */
...
export function ChatInput({
  ...
  const adornmentHelpers = useMemo<ChatInputAdornmentHelpers>(
    () => ({
      insertMention: (item) => editorRef.current?.insertMention(item),
    }),
    [],
  );
  const renderedLeftAdornment = renderLeftAdornment?.(adornmentHelpers);
  const renderedAccessoryTray = renderAccessoryTray?.(adornmentHelpers);
  ...
  // surface routes through the editor's coordinated upload pipeline.
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-3 py-2">
          <ContentEditor
            ...
            mentionIssueProjectId={mentionIssueProjectId}
            enableSlashCommands
        ...
        {(uploadEnabled || projectSelectionEnabled || leftAdornment || renderedLeftAdornment) && (
