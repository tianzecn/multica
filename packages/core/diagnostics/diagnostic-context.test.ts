...
describe("bucketDiagnosticPath", () => {
  ...
  it("templates every workspace detail route", () => {
    ...
    expect(bucketDiagnosticPath("/acme/projects/p1/issues")).toBe(
      "/:slug/projects/:id/issues",
    );
    expect(bucketDiagnosticPath("/acme/conversations/chat-1")).toBe(
      "/:slug/conversations/:id",
    );
    expect(bucketDiagnosticPath("/acme/channels/general")).toBe(
      "/:slug/channels/:id",
    );
  ...
  it("keeps static segments that share a slot with an id", () => {
    ...
    expect(bucketDiagnosticPath("/acme/conversations/new?project=p1")).toBe(
      "/:slug/conversations/new",
    );
  });
